...
output=$(rg "^// Copyright (19|20)[\d]{2} (.+ and )?the Parley Authors( and .+)?$\n^// SPDX-License-Identifier: Apache-2\.0 OR MIT$\n\n" --files-without-match --multiline -g "*.rs" -g "!parley_emoji/**" -g "!parley_tests/tests/emoji/**" .)
...
# Check Apache-only emoji test source files.
output=$(rg "^// Copyright (19|20)[\d]{2} (.+ and )?the Parley Authors( and .+)?$\n^// SPDX-License-Identifier: Apache-2\.0$\n\n" --files-without-match --multiline -g "parley_tests/tests/emoji/**/*.rs" .)

if [ -n "$output" ]; then
	echo -e "The following Apache-only emoji test files lack the correct copyright header:\n"
	echo $output
	echo -e "\n\nPlease add the following header:\n"
	echo "// Copyright $(date +%Y) the Parley Authors"
