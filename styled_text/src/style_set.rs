...
impl<L, P> StyleSet<L, P> {
    ...
    /// Resolves a compact style identifier into borrowed payloads from this table.
    #[must_use]
    pub fn segment_style(&self, id: StyleId) -> SegmentStyle<'_, L, P> {
        let record = self.get_style(id).expect("style id must be interned");
        SegmentStyle {
            id,
            record,
            layout: self
                .get_layout(record.layout_id())
                .expect("layout style id must be interned"),
            paint: self
                .get_paint(record.paint_id())
                .expect("paint style id must be interned"),
        }
    }
}
