...
impl<'a, B: Brush> StyleProperty<'a, B> {
    ...
    ///
    /// let brush: StyleProperty<[u8; 4]> = brush.map(|c| c.components);
    /// assert_eq!(brush, StyleProperty::Brush([0, 0, 0, 255]));
    /// ```
    pub fn map<C: Brush>(self, f: impl FnOnce(B) -> C) -> StyleProperty<'a, C> {
        match self {
            ...
            StyleProperty::FontStack(v) => StyleProperty::FontStack(v),
