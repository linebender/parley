...
use crate::analysis::data::{COMPOSITE, Provider};
...
impl AnalysisDataSources {
    ...
    #[inline(always)]
    pub(crate) fn properties(&self, ch: u32) -> Properties {
        Properties(COMPOSITE.get32(ch))
    }
...
impl Properties {
    ...
    #[inline(always)]
    fn bidi_class(self) -> icu_properties::props::BidiClass {
        #[allow(
            clippy::cast_possible_truncation,
            reason = "bidi class data only occupies BIDI_BITS bits"
        )]
        icu_properties::props::BidiClass::from_icu4c_value(
            self.get(Self::BIDI_SHIFT, Self::BIDI_BITS) as u8,
        )
    }

    /// Returns whether the character is an emoji or pictograph.
    #[inline(always)]
    fn is_emoji_or_pictograph(self) -> bool {
        self.get(
    ...
    #[inline(always)]
    fn is_region_indicator(self) -> bool {
        self.get(
    ...
    #[inline(always)]
    fn is_mandatory_linebreak(self) -> bool {
        self.get(
...
pub(crate) fn analyze_text<B: Brush>(lcx: &mut LayoutContext<B>, text: &str) {
    ...
    boundary_iter
        ...
        .fold(false, |is_mandatory_linebreak, (boundary, ch)| {
            let properties = lcx.analysis_data_sources.properties(ch as u32);
