...
//! ## Scope
//!
//! This crate focuses on mapping `styled_text` + `text_style_resolve` computed styles into Parley
...
//! # Ok::<(), text_style_resolve::ResolveStyleError>(())
//! ```
...
use parley::{FontContext, Layout, LayoutContext, RangedBuilder, StyleProperty};
use styled_text::StyledText;
use text_style_resolve::{ComputedInlineStyle, ResolveStyleError};
...
use crate::convert::{
    to_parley_features, to_parley_font_stack, to_parley_font_style, to_parley_font_weight,
    to_parley_font_width, to_parley_line_height, to_parley_overflow_wrap, to_parley_text_wrap_mode,
    to_parley_variations, to_parley_word_break,
};
...
pub fn build_layout_from_styled_text<T, A, B>(
    ...
    quantize: bool,
    default_brush: B,
) -> Result<Layout<B>, ResolveStyleError>
where
    ...
    B: Brush + Clone,
{
    ...
    builder.push_default(StyleProperty::WordBreak(to_parley_word_break(
        paragraph.word_break(),
    )));
    builder.push_default(StyleProperty::OverflowWrap(to_parley_overflow_wrap(
        paragraph.overflow_wrap(),
    )));
    builder.push_default(StyleProperty::TextWrapMode(to_parley_text_wrap_mode(
        paragraph.text_wrap_mode(),
    )));
...
fn push_inline_defaults<B: Brush + Clone>(
    ...
    builder.push_default(StyleProperty::FontStack(to_parley_font_stack(
        style.font_stack(),
    )));
    builder.push_default(StyleProperty::FontSize(style.font_size_px()));
    builder.push_default(StyleProperty::FontWidth(to_parley_font_width(
        style.font_width(),
    )));
    ...
    builder.push_default(StyleProperty::FontVariations(to_parley_variations(
        style.font_variations(),
    )));
    builder.push_default(StyleProperty::FontFeatures(to_parley_features(
        style.font_features(),
    )));
...
fn push_run_diffs<B: Brush + Clone>(
    ...
    push_if!(
        run.font_stack() != default.font_stack(),
        StyleProperty::FontStack(to_parley_font_stack(run.font_stack()))
    );
    push_if!(
    ...
    push_if!(
        ...
        StyleProperty::FontWidth(to_parley_font_width(run.font_width()))
    );
    push_if!(
        run.font_style() != default.font_style(),
        StyleProperty::FontStyle(to_parley_font_style(run.font_style()))
    );
    push_if!(
        run.font_weight() != default.font_weight(),
        StyleProperty::FontWeight(to_parley_font_weight(run.font_weight()))
    );
    push_if!(
        run.font_variations() != default.font_variations(),
        StyleProperty::FontVariations(to_parley_variations(run.font_variations()))
    );
    push_if!(
        run.font_features() != default.font_features(),
        StyleProperty::FontFeatures(to_parley_features(run.font_features()))
    );
    push_if!(
        run.locale() != default.locale(),
        StyleProperty::Locale(run.locale())
    );
    push_if!(
